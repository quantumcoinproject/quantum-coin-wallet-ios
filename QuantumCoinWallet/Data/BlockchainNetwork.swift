...
// `SECURE_VAULT_BLOB` once the wallet is unlocked, and re-points
...
public struct BlockchainNetwork: Codable, Equatable {
    ...
    // and the encrypted vault, AND Android-style keys (`networkId`,
    ...
    // the vault format on disk does not change.
    //
    ...
    // legacy vault that still has `id` is simply ignored on decode.
    ...
    /// vault stays binary-compatible with existing on-disk user data.
...
public final class BlockchainNetworkManager {
    ...
    /// Called from `KeyStore.unlock` once the encrypted vault blob has
    /// been decrypted. Layers the user-added networks on top of the
    ...
    /// Called from `KeyStore.lockWallet` so a foreground after relock
    /// only sees the bundled MAINNET (mirrors how the address map
    ...
    /// `KeyStore` can re-derive the vault main key, re-encrypt the
    /// vault blob with the new active-index, and zero the bytes before
    ...
    public func setActive(index: Int, password: String) throws {
        ...
        activeIndex = index
        do {
            try persistThroughKeyStore(password: password)
        } catch {
    ...
    /// entry must be written to the encrypted vault blob, which means
    /// re-deriving the vault main key from the user's password. On
    ...
    /// stricter: the same data goes into the encrypted vault blob
    /// (`SECURE_VAULT_BLOB`) alongside the address map. The encryption
    /// key (`mainKey`) is intentionally NOT cached in memory across
    /// operations - every vault write derives it from the user's
    /// password, uses the bytes once, and zeroes them. So the picker
    /// prompts the user for their password through
    /// `UnlockDialogViewController` before calling `addNetwork` or
    /// `setActive`, which forwards the password to
    /// `KeyStore.recordNetworks` for the actual derive-encrypt-write
    /// cycle.
    private func persistThroughKeyStore(password: String) throws {
        let bundledCount = loadBundled().count
        let custom = Array(networks.dropFirst(bundledCount))
        try KeyStore.shared.recordNetworks(
